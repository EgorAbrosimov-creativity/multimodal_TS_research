...
class Model(nn.Module):
    """
    Late Fusion: independent PatchTST + BERT text branch, fused at the output.

    Pipeline:
        ts_out   = PatchTST(x_enc)             [B, pred_len, enc_in]
        text_out = BERT-MLP(x_enc)             [B, pred_len, enc_in]
        out      = Linear(concat([ts_out, text_out], dim=-1))
    """
    ...
    def __init__(self, configs, patch_len=16, stride=8):
        ...
        # ── Fusion ────────────────────────────────────────────────────────
        self.fusion = nn.Linear(configs.enc_in * 2, configs.enc_in)

    def _ts_encode(self, x_enc):
        # Normalization
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        ts_out = self._ts_encode(x_enc)                       # [B, pred_len, enc_in]
        ...
        combined = torch.cat([ts_out, text_out], dim=-1)      # [B, pred_len, 2*enc_in]
        out = self.fusion(combined)                            # [B, pred_len, enc_in]
        return out
